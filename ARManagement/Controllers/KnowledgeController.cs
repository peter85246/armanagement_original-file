    ...
    public class KnowledgeController : MyBaseApiController
    {
        ...
        public async Task<ActionResult<ApiResult<List<KnowledgeBase>>>> GetAllKnowledgeBaseByFilter(PostKnowledgeinfoFilter post)
        {
            ...
            try
            {
                ...
                {
                    where += @" AND (""KnowledgeBaseDeviceType"" LIKE CONCAT('%', @Keyword ,'%') OR ""KnowledgeBaseDeviceParts"" LIKE CONCAT('%', @Keyword ,'%') )";
                }

                var knowledgeBase = await _baseRepository.GetAllAsync<KnowledgeBase>("KnowledgeBase", where, new { Keyword = post.Keyword }, "\"KnowledgeBaseId\" ASC");
        ...
        public async Task<ActionResult<ApiResult<int>>> GetLatestMachineAddId()
        {
            ...
            try
            {
                // 使用第二個 GetOneAsync 方法獲取最新的MachineAddId
                string tableName = "MachineAdd";
                string sWhere = "\"Deleted\" = 0"; // 假設有一個Deleted字段來標記記錄是否被刪除
                string selCol = "\"MachineAddId\""; // 您想要返回的列
                string sOrderBy = "\"MachineAddId\" DESC"; // 降序排序以獲取最新的ID
                ...
                if (latestMachineAddId != 0)
                {
                    apiResult.Code = "0000";
                    apiResult.Message = _responseCodeHelper.GetResponseCodeString(apiResult.Code);
                    return Ok(latestMachineAddId); // 返回最新的MachineAddId
                }
                else
                {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult<ApiResult<List<KnowledgeBase>>>> GetAllKnowledgeBaseByMachineAddId([FromForm] PostAllKnowledgeBase post)
        {
            ApiResult<List<KnowledgeBase>> apiResult = new ApiResult<List<KnowledgeBase>>(); /*jwtToken.Token*/
            ...
            try
            {
                ...
                foreach (var knowledgeBase in knowledgeBases)
                {
                    ...
                    {
                        knowledgeBase.KnowledgeBaseModelImage = $"{baseURL}upload/machineAdd/{machineAdd.MachineAddId}/knowledgeBase/{knowledgeBase.KnowledgeBaseId}/{knowledgeBase.KnowledgeBaseModelImage}";
                    }

                    if (!string.IsNullOrEmpty(knowledgeBase.KnowledgeBaseToolsImage))
                    {
                        knowledgeBase.KnowledgeBaseToolsImage = $"{baseURL}upload/machineAdd/{machineAdd.MachineAddId}/knowledgeBase/{knowledgeBase.KnowledgeBaseId}/{knowledgeBase.KnowledgeBaseToolsImage}";
                    }

                    if (!string.IsNullOrEmpty(knowledgeBase.KnowledgeBasePositionImage))
                    {
                        knowledgeBase.KnowledgeBasePositionImage = $"{baseURL}upload/machineAdd/{machineAdd.MachineAddId}/knowledgeBase/{knowledgeBase.KnowledgeBaseId}/{knowledgeBase.KnowledgeBasePositionImage}";
                    }
